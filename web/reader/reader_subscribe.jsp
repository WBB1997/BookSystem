...
<script type="text/html" id="statusTpl">
    {{#  if(d.Status === '已预约'){ }}
    <div align="center">
        <lable style="color: #fb830e">已预约</lable>
    </div>
    ...
    <div align="center">
        <lable style="color: green">已完成</lable>
    </div>
...
<script>
    layui.use(['form', 'laypage', 'layer', 'table', 'upload', 'element'], function () {
        ...
        table.render({
            ...
            , id: 'bookTable'
            , height: 'full-100'
            , url: '<%=request.getContextPath()%>/ReaderServlet?action=getReaderBookSubscribe' //数据接口
        ...
        table.on('tool(bookTable)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'cancel') {
                layer.confirm('确定取消？', function (index) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: '<%=request.getContextPath()%>/ReaderServlet?action=cancelBookSubscribe',
                        data: {
        ...
        $("#searchId").click(function () {
            ...
            table.reload('bookTable', {
                url: '<%=request.getContextPath()%>/ReaderServlet?action=getReaderBookSubscribe&keyword=' + searchWord
            });
        });
        ...
        function flushTab() {
            ...
            table.reload('bookTable', {
                url: '<%=request.getContextPath()%>/ReaderServlet?action=getReaderBookSubscribe'
            });
        }
