...
<script>
    layui.use(['form', 'laypage', 'layer', 'table', 'upload', 'element'], function () {
        ...
        table.render({
            ...
            , id: 'bookTable'
            , height: 'full-100'
            , url: '<%=request.getContextPath()%>/ReaderServlet?action=getReaderRequestHistory' //数据接口
        ...
        table.on('tool(bookTable)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'cancel') {
                layer.confirm('确定取消？', function (index) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: '<%=request.getContextPath()%>/ReaderServlet?action=cancelBookRequest',
                        data: {
        ...
        $("#searchId").click(function () {
            ...
            table.reload('bookTable', {
                url: '<%=request.getContextPath()%>/ReaderServlet?action=getReaderRequestHistory&keyword=' + searchWord
            });
        });
        ...
        function flushTab() {
            ...
            table.reload('bookTable', {
                url: '<%=request.getContextPath()%>/ReaderServlet?action=getReaderRequestHistory'
            });
        }
