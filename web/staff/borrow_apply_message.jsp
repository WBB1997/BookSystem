...
<script>
    layui.use(['form', 'laypage', 'layer', 'table', 'upload', 'element'], function () {
        ...
        table.render({
            ...
            , id: 'bookTable'
            , height: 'full-100'
            , url: '<%=request.getContextPath()%>/StaffServlet?action=getBorrowApplyHistory' //数据接口
        ...
        table.on('tool(bookTable)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'ok') {
                layer.confirm('请仔细审核读者信息，是否同意？', function (index) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: '<%=request.getContextPath()%>/StaffServlet?action=dealWithBorrowApply&keyword="同意"',
                        data: {
                ...
                layer.confirm('是否拒绝？', function (index) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: '<%=request.getContextPath()%>/StaffServlet?action=dealWithBorrowApply&keyword="拒绝"',
                        data: {
                            isbn: data.ISBN//传向后端的数据
                            , account: data.Reader_No//传向后端的数据
                        },
        ...
        $("#searchId").click(function () {
            ...
            table.reload('bookTable', {
                url: '<%=request.getContextPath()%>/StaffServlet?action=getBorrowApplyHistory&keyword=' + searchWord
            });
        });
        ...
        function flushTab() {
            ...
            table.reload('bookTable', {
                url: '<%=request.getContextPath()%>/StaffServlet?action=getBorrowApplyHistory'
            });
        }
