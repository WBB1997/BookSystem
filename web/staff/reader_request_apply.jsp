...
<script type="text/html" id="bookBar">
    ...
    <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="ok">已到馆</a>
    {{#  }}}
</script>
...
<script type="text/html" id="statusTpl">
    ...
    {{#  }else if (d.Status === '荐购失败'){ }}
    <div align="center">
        <lable style="color: red">荐购失败</lable>
    </div>
    {{#  }else if(d.Status === '已到馆'){ }}
    <div align="center">
        <lable style="color: green">已到馆</lable>
    </div>
...
<script>
    layui.use(['form', 'laypage', 'layer', 'table', 'upload', 'element'], function () {
        ...
        table.render({
            ...
            , id: 'bookTable'
            , height: 'full-100'
            , url: '<%=request.getContextPath()%>/StaffServlet?action=getBookRequestHistory' //数据接口
            ...
            , cols: [[ //表头
                {field: 'No', align: 'center', title: '读者账号'}
        ...
        table.on('tool(bookTable)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'ok') {
                layer.confirm('请仔细审核读者信息，是否同意？', function (index) {
                    $.ajax({
                        type: 'get',
                        url: '<%=request.getContextPath()%>/StaffServlet?action=dealWithBookRequest&keyword="同意"',
                        data: {
                            isbn: data.ISBN//传向后端的数据
                            , account: data.No//传向后端的数据
                        },
                ...
                layer.confirm('是否拒绝？', function (index) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: '<%=request.getContextPath()%>/StaffServlet?action=dealWithBookRequest&keyword="拒绝"',
                        data: {
                            isbn: data.ISBN//传向后端的数据
                            , account: data.No//传向后端的数据
                        },
        ...
        $("#searchId").click(function () {
            ...
            table.reload('bookTable', {
                url: '<%=request.getContextPath()%>/StaffServlet?action=getBookRequestHistory&keyword=' + searchWord
            });
        });
        ...
        function flushTab() {
            ...
            table.reload('bookTable', {
                url: '<%=request.getContextPath()%>/StaffServlet?action=getBookRequestHistory'
            });
        }
