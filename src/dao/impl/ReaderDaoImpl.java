...
public class ReaderDaoImpl implements IReaderDao {
    ...
    public void registerAccount(Reader r, SqlStateListener l) {
        ...
        try {
            ...
            CallableStatement callableStatement = connection.prepareCall("{call register_Account_Of_Reader(?, ?, ?, ?, ?, ?)}");
    ...
    public void modAccount(Reader r, SqlStateListener l) {
        ...
        try {
            connection = DruidManager.getInstance().getConnection();
            CallableStatement callableStatement = connection.prepareCall("{call mod_Account_Of_Reader(?, ?, ?, ?, ?, ?)}");
            fillPara_6(callableStatement, r);
            callableStatement.execute();
            l.Correct();
        } catch (SQLException e){
    ...
    public void delAccount(Reader r, SqlStateListener l) {
        ...
        try {
            connection = DruidManager.getInstance().getConnection();
            CallableStatement callableStatement = connection.prepareCall("{call del_Account_Of_Reader(?, ?)}");
    ...
    public void borrowBook(Reader r, Book b, SqlStateListener l) {
        ...
        try {
            connection = DruidManager.getInstance().getConnection();
            CallableStatement callableStatement = connection.prepareCall("{call Borrow_Book(?, ?, ?)}");
