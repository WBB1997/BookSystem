...
public class BookDaoImpl implements IBookDao {
    ...
    public void increaseBook(Book b, int quantity, Admin a, SqlStateListener l) {
        ...
        try {
            connection = DruidManager.getInstance().getConnection();
            CallableStatement callableStatement = connection.prepareCall("{call increase_Book(?, ?, ?, ?)}");
    ...
    public void decreaseBook(Book b, int quantity, Admin a, SqlStateListener l) {
        ...
        try {
            connection = DruidManager.getInstance().getConnection();
            CallableStatement callableStatement = connection.prepareCall("{call decrease_Book(?, ?, ?, ?)}");
            fillPara_4(callableStatement, b, quantity, a);
            callableStatement.execute();
            l.Correct();
        } catch (SQLException e) {
    ...
    public void modBook(Book b, Admin a, SqlStateListener l) {
        ...
        try {
            connection = DruidManager.getInstance().getConnection();
            CallableStatement callableStatement = connection.prepareCall("{call mod_Book(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}");
            fillPara_11(callableStatement, a, b);
            callableStatement.execute();
            l.Correct();
        } catch (SQLException e) {
    ...
    @Override
    public List<Book> queryAllBook(String word) throws Exception {
        Connection connection = DruidManager.getInstance().getConnection();
    ...
    @Override
    public List<Book> queryBookOfKeyword(String parameter, String word) throws Exception {
